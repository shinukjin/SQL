...
-- DROP TABLE T2_TABLE; 테이블 삭제
SELECT * FROM T2_TABLE;

--테이블에 인서트하기
--INSERT INTO : 인서트 키워드
--T2_TABLE : 테이블명
--(T2_COLUMN1, T2_COULMN2) : 컬럼명을 , 으로 순차로 명기하기
--VALUES : 키워드
...
--오라클에서 ISUD, CRUD 할 때 주의해야 할것!!!!!
--SELECT
-- DDL : 트랜잭션을 처리해야하는 명령어
-- 오라클에선 트랜잭션은 해당하는 ROW를 COMMIT, ROLLBACK 하는 것이다.
-- 오라클에선 DDL 문 INSERT, UPDATE, DELETE 에서만 트랜잭션만 개발자가 관리해야한다. 
-- COMMIT, ROLLACK 을 직접해야한다.

-- INSERT, UPDATE, DELETE
-- 인서트 업데이트 딜리트 명령어로 트랜잭션을 발생 시키면
-- 1. 인서트, 업데이트, 딜리트한 내용이 오라클 메모리에 작성된다.
-- 2. 이 상태에서 SELECT 하면 조회가 된다.
--    sqlplus에서는 조회가 되는데, 외부 응용프로그램에서는 조회가 불가능 하다.
-- 2-1. 인서트, 업데이트, 딜리트 한 내용 오라클 메모리에서 삭제하려면 ROLLBACK 명령어를 실행하면 된다.
-- 2-2. 인서트, 업데이트, 딜리트 후 조회가 가능한데
--     COMMIT 명령어를 실행하지 않고 ROLLBACK 명령어를 실행하면 조회가 안된다.
-- 3. COMMIT 명령어를 실행시켜야 오라클 파일에 작성된다.
-- 3-1. COMMIT 명령이 실행 된 이후에는 ROLLBACK이 되지 않는다.
-- 4. 이 상태에서 sqlplus 및 외부 응용프로그램에서 조회가 가능하다.
-- 4-1. 외부 응용프로그램에서 오라클테이블에 있는 데이터를 조회하려면 
--     파일에 적재된 데이터만 조회가 가능하다.
--     이유는 메모리에 있는 내용은 변경이 가능하기 때문에

INSERT INTO T2_TABLE (T2_COLUMN1, T2_COLUMN2) VALUES ('111','111');
SELECT * FROM T2_TABLE;
ROLLBACK; --COMMIT 한 다음에는 롤백되지 않는다.
COMMIT;
...
CREATE TABLE T3 (
        ...
        , T3_2 VARCHAR2(20)
        , T3_3 VARCHAR2(20)
        , T3_4 NUMBER(7,4) --소수점 포함 7자리 1111은 1111.0000으로 봐서 FAIL
        , T3_5 VARCHAR2(1)
        , T3_6 DATE
);
SELECT * FROM TAB;
...
--
--오류 보고 -
--ORA-00001: 무결성 제약 조건(TEST02.SYS_C0011116)에 위배됩니다
